...
整页只有一个地方需要水合——阅读进度条和主题切换这样的小组件。首屏 JS 体积因此可以忽略不计，Lighthouse 的性能分不再是玄学。性能之外，[前端细节](/articles/frontend-details-that-matter)同样是被当作产品的一部分来对待的，而不是发布前的最后一层装饰。
...
如果只是想写博客，这套架构是过度工程。但如果你和我一样，把个人站当作**新技术的低风险试验田**——在这里踩 RSC、边缘计算、OG 图自动生成的坑，比在生产环境里第一次遇到它们便宜得多。这也是我一直说的：[先把系统做小，再把价值做大](/articles/build-small-systems)。
