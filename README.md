...
npm run lint       # oxlint
npm test           # node --test（内容解析 / 集合查询 / 路由输出，零新依赖）
...
- **字体分片**：`split-fonts.py` 把 Noto Serif SC 拆成 unicode-range 分片（**31 片**，数量由 `npm run check:glyphs` 盯住），输出到 `public/fonts/slices/`，文件名带内容哈希。**每个字重有一个关键片**（`s0`），覆盖首页全部衬线文字，由 `layout.tsx` 预加载并并行下载——实测首屏只需 3 片 / 97 KB。脚本会直接改写 `app/globals.css` 的 `@font-face` 块与 `lib/font-slices.generated.ts`，采集口径与自检写在脚本头部。`fonts-src/` 是它的输入（本地中间产物，不进 git），`subset-fonts.py` 负责从完整 OTF 生成整包。
...
4. 站点用字有变化时（新增长文、新栏目）：`npm run fonts`。它会重出字体分片、直接改写 `app/globals.css` 的 `@font-face` 块与 `lib/font-slices.generated.ts`（preload 清单），并自检分片不重叠。若脚本提示「某标题用字不在 w500 关键片内」，按脚本头部说明重新采集 `CRITICAL_BY_WEIGHT` 再跑一次，然后 `npm run build`。上游整包子集由 `subset-fonts.py` 生成，输入放在 `fonts-src/`。
   ...
   **注意**：`npm run build` 现在会先跑 `npm run check`。正文里出现字体分片没声明的字时，构建会**直接失败并列出缺字**（以及修复命令）——这是刻意的：缺字形在浏览器里不会报错，只会静默回退到访客本机字体，衬线标题里混进宋体。
