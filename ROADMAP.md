...
**渐变依然成立，但性质变了**：亮色下是「从亮到中」的压暗，暗色下是「从无色到有色」的染色。实测渐变路径 `rgb(26,26,29) → rgb(82,68,146)`，方向依然清晰。

亮色模式**完全未变**（`--cta-violet: var(--violet)` 就是原值），线上复测确认。

### 截图验证踩的坑（内嵌浏览器）

给 CTA 截图比对时绕了很久，值得记下：

- **可见表面比布局视口小**。`Emulation.setDeviceMetricsOverride` 能让 `window.innerWidth` 报 1280、让 `getBoundingClientRect` 按 1280 算，但**渲染表面仍是面板宽度**，截图右侧/下方是黑的。
- **`page.screenshot({clip})` 的 x/y 是文档坐标**，不是视口坐标。拿视口坐标去 clip 会截到页面顶部——第一次就这样采到一片纯黑，还差点误判成「渐变没生效」。
- **`zoom: 0.45` 能把宽屏布局塞进小面板**（布局视口变成 `可见宽度 / 0.45`），但 `getBoundingClientRect` 在 zoom 下的语义不一致（块级元素返回未缩放值、内联元素返回缩放值），`scrollIntoView` 会飘到视口外。
- **图片被缩放显示时会骗人**：1600px 宽的截图压进面板后，深紫看起来像亮紫。**判断颜色一律用像素采样，不要靠看图**。

最终靠「注入 CSS → 采样像素」交叉确认：`#4d3fa6` 渲染为 `rgb(82,68,146)`，`#9186f0` 渲染为亮紫。
